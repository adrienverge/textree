  ...
  return Q.nfcall(require("fs").readFile, headFile, { encoding: "utf-8" })
    ...
    .then(function(repos) {
      ...
      return gitRef.length == 40 ?
        Q.nfcall(repos.getCommit.bind(repos), gitRef) :
        Q.nfcall(repos.getBranch.bind(repos), gitRef);
    })
    .then(function(commit) {
      console.error("Loaded GIT commit", gitRef, ":", commit.sha());
      gitCommit = commit;
      return Q.nfcall(commit.getTree.bind(commit));
      // return gitCommit.getTree();
