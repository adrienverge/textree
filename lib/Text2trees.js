...
Text2trees.prototype._transform = function (chunk, encoding, done, arg) {
  ...
  if (inc > 0 && chunk.length > inc) {
    ...
    function loop() {
      ...
      Q.when(_this.processor()).then(function() {
        if (_this._processorReset && !wasDone) {
          loop();
        }
      });
    };
...
Text2trees.prototype.elementBreakChars = ">|.#@( \n";

Text2trees.prototype.goForNewLine = function() {
  var _this = this;
  this.currentLine++;
  // log("new line", this.pos, _s(this.buffer));
  // this.goFor(function() {
  // log("NEW LINE");
...
Text2trees.prototype.goAfterToken = function (token) {
  // log("indent="+_this.lineIndent, "token", _s(token), "from", _s(_this.buffer));
  var _this = this;
  if (!token) {
    ...
    switch (c) {
     ...
     case "|":
      ...
      _this.pos++;
      return _this.goForText();
    }
  }
  if (/^-#|^\/\//.test(token + _this.buffer[_this.pos])) {
    _this.push({type: "comment", value: token + _this.buffer[_this.pos] });

    return _this.goForIgnoredLine();
  } else {
    // log(_this.lineIndent, _this.hamlNode && _this.hamlNode.lineIndent);
    // token
    _this.returnToIndent(_this.lineIndent);
...
Text2trees.prototype.goForAnyAttr = function() {
  ...
  if (c == "(") {
    return this.goForAttribute();
  }
  if (c == " ") {
    return this.goForText();
  }
  // this.readTokenUntil("")
  // });
};
...
Text2trees.prototype.goForText = function() {
  ...
  return this.readTokenUntil("\n")
    .then(function(text) {
      _this.push({type: "text", text: text.trim() });
      _this.pos++;
      return _this.goForNewLine();
    });
};
