...
Text2trees.prototype.goForCommentLine = function(commentToken) {
  ...
  this.readTokenUntil("\n")
    .then(function(comment) {
      ...
      _this.push({type: "comment", value: commentToken + comment });
      _this.idx++;
      _this.goForNewLine();
    }).done();
};
...
Text2trees.prototype.goForAnyAttr = function() {
  ...
  if (c == "." || c == "#") {
    this.readTokenUntil(".#@( }\n"/*=*/).then(function(token) {
      ...
      if (c == "#") {
        _this.push({type: "attr", "name": "id", value: token });
        // _this.push({type: "id", value: token });
      } else if (c == "}") {
        this.goWithInlineChar(c);
      } else {
        _this.push({type: "attr", "name": "class", value: token });
...
Text2trees.prototype.goForAttribute = function() {
  ...
  this.readTokenUntil("=")
    ...
    .then(function(token) {
      _this.push({type: "attr", name: attr, value: token.trim() });
      var c = _this.buffer[_this.pos];
      _this.pos++;
      if (c == ")") {
