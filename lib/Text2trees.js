...
/**
 * Text2trees: textree source parser to event objects
 *
 ...
 *
 */
Text2trees.prototype.sendEvent = function (event) {
...
Text2trees.prototype.getLastStartEvent = function () {
  if (!this.lastStartEvent) {
    console.warn("Text2trees#getLastStartEvent: no last start event!");
  }
 ...
 */
Text2trees.prototype.goForCommentLine = function(commentToken) {
  var _this = this;

  this.readTokenUntil("\n")
    .then(function(comment) {
      // console.log("lalala", arguments);
      _this.sendEvent({type: "comment", value: commentToken + comment });
      _this.idx++;
      _this.goForNewLine();
    }).done();
};
  ...
  if (c == "." || c == "#") {
    this.readTokenUntil(".#@( }\n"/*=*/).then(function(token) {
      ...
      if (c == "#") {
        ...
        var attrs = _this.getLastStartEvent().attributes;
        attrs["class"] = ("class" in attrs ? (attrs["class"]+" ") : "") + token;
      }
...
Text2trees.prototype.goForText = function() {
  ...
  //   text = text.trim();
  //   this.sendEvent({type: "text", text: text.trim() });
  // }
  this.readTokenUntil("{}\n")
    .then(function(text) {
      // text = text.trim();
