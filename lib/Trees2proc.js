  ...
  this.captureFirstText()
    ...
    .then(function(buffer /* of events to apply when selector is met */) {
      ...
      this.addMatch(selector, function(event) {
        ...
        // console.log("mask", vars, this.vmContext);
...
Trees2proc.prototype.processPath = function () {
  // this.captureLevelEvents(true)
  //   .then(function() {
  //   });
...
Trees2proc.prototype.sourceFile = function (path) {
  // env.getPath(path).then(function
  var catFile = new CatFile();
  var text2trees = new Text2trees();

  // var side = this.transformChain.getSide("<", this);
  // // console.log("side", side);
  // if (side) {
  //   side.unpipe(this);
  // }
  // text2trees.pipe(this);
  ...
  return env.getPath(path)
    ...
    .then(function(blob) {
      // this.transformChain.connectSide("<", this, catFile);
      text2trees.end(blob);
    });

  // catFile.write({ path: path });

  // this.transformChain.connectSide("<", this, null);
  // env.getPath(path).then(
  //     function(gitEntry) {
  //       entry.present = true;
  //       entry.type = entryPath.isTree() ? "directory" : "file";
  //     }.bind(this),
  //     function(error) {
 ...
 */
Trees2proc.prototype.captureLevelEvents = function (dontBuffer) {
  var buffer = [];
  var def = Q.defer();
  var depth = 0;
