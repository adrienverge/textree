...
Trees2proc.prototype.addMatch = function (selector, handler, parentLevel) {
  this.debugInfo("addMatch", selector, parentLevel);
...
Trees2proc.prototype.command_through = function (event) {
  console.log("through pos=", this.throughPos, this.eventQueue.length, this.eventQueue);
  if (this.throughPos === undefined) {
    console.log("WARN: this.throughPos is undefined");
  } else {
    this.captureSubLevelEvents()
      .then(function() {
        // console.log("***", this.eventQueue.length - this.throughPos, this.eventQueue);
        this.debugInfo("this.eventQueue1111");
        var buffer = this.eventQueue.splice(0, this.eventQueue.length - this.throughPos);
        console.log("EXTRACTED buf", buffer);
        // console.log("this.eventQueue", this.eventQueue);
        this.debugInfo("this.eventQueue 2222");
        // while (!buffer[0].type == "end") {
        //   buffer.shift();
        // }
        //   buffer.shift();
        ...
        this.debugInfo("this.eventQueue END2");
        }.bind(this);
      }.bind(this))
      .done();
  }
