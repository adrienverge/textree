...
Trees2proc.prototype.command_on = function (event) {
  ...
  this.captureFirstText()
    ...
    .then(function(text) {
      ...
      var selector = (""+parse[1]).trim();
      var bindName = (""+(parse[3] || selector)).trim();
    ...
    .then(function(templateBuffer /* of events to apply when selector is met */) {
      ...
      var handler = function(event, selectors) {
        ...
        var templateReadable = new ArrayPrependStream(templateBuffer);
        this.transformChain.connectSide("<", this, templateReadable);
...
Trees2proc.prototype.command_on = function (event) {
  this.captureFirstText()
    .then(function(text) {
      // ...
      this.addMatch(selector, function() {
      });
    });
};
  ...
  this.captureFirstText()
    .then(function(text) {
      if (/^-/.test(text)) {
        ...
        this.transformChain.connectSide("<", this, new LevelStream(this));
