...
Trees2proc.prototype.command_on = function (event) {
  ...
  this.captureFirstText()
    ...
    .then(function(text) {
      ...
      var parse = /([a-zA-Z_\$][a-zA-Z_\$]*) *([:+]*=) *(.*)/.exec(text);
      ...
      //   if ((uniqueAffect = /:$/.test(bindName))) {
      //     bindName = bindName.substr(0, bindName.length - 1);
      //   }
      //   bindName = bindName.trim();
      // }
      // selector = parts[0].trim();

      console.log("selector", selector, "bind", bindName, "directChild", directChild, "affect", affect);
      return this.captureLevelEvents();
    }.bind(this))
    ...
    .then(function(buffer /* of events to apply when selector is met */) {
      ...
      var handler = function(event, selectors) {
        ...
        switch (event.type) {
         ...
         case "start":
          if (wholeTree) {
            this.captureLevelEvents().then(apply);
          } else {
            apply(event);
          }
          break;
